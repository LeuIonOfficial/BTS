...
const CreateUserDrawer: React.FC<Props> = ({ drawerState, setDrawerState, userToUpdate }) => {
  const [form] = useForm();
  ...
  const handleFinish = (values: PostUserType) => {
    ...
    drawerState === 'update' && userToUpdate && updateUser({ ...values, id: userToUpdate.id });
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center">
        ...
        <Form layout="horizontal" form={form} onFinish={handleFinish} name="user">
        ...
        <h1 className="text-[#e6e6e6] text-[180px] font-[800] tracking-[-10px] mb-0">BTS</h1>
      </div>
    </Drawer>
  );
};
